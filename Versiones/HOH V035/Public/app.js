...
function showDetail(n) {
    ...
    $('detailContent').innerHTML = `<h2>${t.number}</h2>${stateBadge(t.state||'')}<p><b>SAP:</b> ${cs} <button class='action-btn' onclick="copyText('${cs}')">📋 Copiar SAP</button></p><p><b>INC:</b> ${t.number} <button class='action-btn' onclick="copyText('${t.number}')">📋 Copiar INC</button></p><p><b>Edad:</b> ${age(t.sys_created_on)}</p><p class='strike${strike(h)}'><b>Strike ${strike(h)}</b></p><p><b>${sla(h)[0]}</b></p><h3>Duplicados Activos (${active.length})</h3>${active.length?active.map(x=>`<div class='dup-row'><span>${String(x.state||'').toLowerCase().includes('espera')?'🟡':'🔴'}</span><span>${x.state||''}</span><span>${x.number}</span><span>${fmt(x.sys_created_on)}</span></div>`).join(''):'<div class="empty-dup">✅ Sin duplicados activos</div>'}<h3>Histórico SAP</h3>${hist.length?hist.map(x=>`<div class='history-row'><span>✅</span><span>${x.state||''}</span><span>${x.number}</span><span>${fmt(x.sys_created_on)}</span></div>`).join(''):'<div class="empty-dup">Sin historial</div>'}<h3>Timeline</h3><div class='timeline-item'>Ticket creado</div><div class='timeline-item'>Asignado</div><div class='timeline-item'>Estado actual: ${t.state||''}</div>`
}
